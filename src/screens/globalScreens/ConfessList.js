  ...
  return (
    <Background>
      <LinearGradient
        ...
        colors={['#9f4c4cff', 'transparent']}
      >
        <Text style={{ fontWeight: 'bold', fontSize: 22, color: 'white' }}>
          {DeviceLanguage === 'ar' ? 'المواضيع' : 'İtiraflar'}
        </Text>
        ...
        >
          <MaterialCommunityIcons name="pencil" size={20} color="#9f4c4cff" />
          <Text style={{ color: '#9f4c4cff', fontWeight: 'bold', marginLeft: 5 }}>
            {DeviceLanguage === 'ar' ? 'جديد' : 'İtiraf Yaz'}
          </Text>
        </TouchableOpacity>
      </LinearGradient>
      <View style={{ width: "100%", alignItems: "center", backgroundColor: "#ece1e1ff", padding: 5, borderWidth: 1, borderColor: "#9f4c4cff" }}>
        <Text style={{ fontSize: 16, fontWeight: "bold", color: "#9f4c4cff" }}>İtiraf et rahatla! İtiraf et huzuru yakala!</Text></View>
      <ScrollView
      ...
      >
        {topics.length === 0 ? (
          ...
          topics.map((topic, index) => (
            <TouchableOpacity
              key={topic.id || index}
              onPress={() => props.navigation.navigate('ConfessDetail', { topicId: topic.id })}
              style={{
                backgroundColor: 'white',
                margin: 10,
                padding: 15,
            ...
            >
              <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 15, color: '#9f4c4cff' }}>
                {topic.title}
              </Text>
              {/* <Text
